...
fun PaymentScreen(
    ...
    trackingRepository: TrackingRepository = koinInject()  // Phase 6: Tracking
) {
                ...
                coroutineScope.launch {
                    ...
                    try {
                        ...
                        createOrderUseCase(order)
                            .onSuccess { orderId ->
                                // Phase 6: Track conversion (purchase from affiliate link)
                                coroutineScope.launch {
                                    try {
                                        // orderId is String, need to convert if backend expects Int
                                        val orderIdInt = orderId.toIntOrNull() ?: 0
                                        if (orderIdInt > 0) {
                                            trackingRepository.trackConversion(orderIdInt)
                                            Log.d("PaymentTracking", "✅ Conversion tracked for order $orderId")
                                        }
                                    } catch (e: Exception) {
                                        Log.e("PaymentTracking", "❌ Failed to track conversion: ${e.message}")
                                    }
                                }
